...
var require_native = __commonJS({
  "node_modules/rollup/dist/native.js"(exports, module) {
    ...
    var { platform: platform2, arch, report } = require_node_process();
    ...
    if (!packageBase) {
      throw new Error(
        `Your current platform "${platform2}" and architecture "${arch}" combination is not yet supported by the native Rollup build. Please use the WASM build "@rollup/wasm-node" instead.
...
If this is important to you, please consider supporting Rollup to make a native build for your platform and architecture available.`
    ...
    function getPackageBase() {
      var _a;
      const imported = (_a = bindingsByPlatformAndArch[platform2]) == null ? void 0 : _a[arch];
      if (!imported) {
        return null;
      }
    ...
    var { parse: parse3, parseAsync: parseAsync2, xxhashBase64Url: xxhashBase64Url2 } = requireWithFriendlyError(
...
function error(base2) {
  if (!(base2 instanceof Error)) {
    ...
    Object.defineProperty(base2, "name", { value: "RollupError", writable: true });
  }
  throw base2;
}
...
var import_native2 = __toESM(require_native(), 1);
...
var ExternalChunk = class {
  ...
  getFileName() {
    ...
    return this.fileName = (typeof paths === "function" ? paths(this.id) : paths[this.id]) || (this.renormalizeRenderPath ? normalize((0, import_node_path2.relative)(this.inputBase, this.id)) : this.id);
  }
  getImportAttributes(snippets) {
    return this.importAttributes || (this.importAttributes = formatAttributes2(this.options.format === "es" && this.options.externalImportAttributes && this.moduleInfo.attributes, snippets));
  }
...
var NodeBase = class extends ExpressionEntity {
  ...
  constructor(esTreeNode, parent, parentScope, keepEsTreeNode = false) {
    ...
    keys[type] || (keys[type] = createKeysForNode(esTreeNode));
    this.parent = parent;
  ...
  parseNode(esTreeNode, keepEsTreeNodeKeys) {
    for (const [key, value] of Object.entries(esTreeNode)) {
      ...
      if (key.charCodeAt(0) === 95) {
        ...
        this[key] = [];
        for (const child of value) {
          this[key].push(child === null ? null : new (this.scope.context.getNodeConstructor(child.type))(child, this, this.scope, keepEsTreeNodeKeys == null ? void 0 : keepEsTreeNodeKeys.includes(key)));
        }
      } else {
        this[key] = new (this.scope.context.getNodeConstructor(value.type))(value, this, this.scope, keepEsTreeNodeKeys == null ? void 0 : keepEsTreeNodeKeys.includes(key));
      }
    }
  }
...
var Scope = class {
   ...
   */
  addDeclaration(identifier, context, init2, kind) {
    var _a;
    ...
    const newVariable = new LocalVariable(identifier.name, identifier, init2, context, kind);
    ...
    if (kind === "var") {
      ...
      if (existingVariable) {
        ...
        existingVariable.declarations[0].parent.type === CatchClause$1) {
          const declaredVariable2 = this.parent.parent.addDeclaration(identifier, context, UNDEFINED_EXPRESSION, kind);
      ...
      const declaredVariable = this.parent.parent.addDeclaration(identifier, context, init2, kind);
      declaredVariable.markInitializersForDeoptimization();
      this.addHoistedVariable(name, declaredVariable);
      return declaredVariable;
    }
    return super.addDeclaration(identifier, context, init2, kind);
  }
};
...
var BlockScope = class extends ChildScope {
  ...
  addDeclaration(identifier, context, init2, kind) {
    ...
    if (kind === "var") {
      ...
      const declaredVariable = this.parent.addDeclaration(identifier, context, init2, kind);
      declaredVariable.markInitializersForDeoptimization();
      this.addHoistedVariable(name, declaredVariable);
      return declaredVariable;
    }
...
var ClassBodyScope = class extends ChildScope {
  constructor(parent, classNode) {
    ...
    this.variables.set("this", this.thisVariable = new LocalVariable(
      "this",
      null,
      classNode,
      context,
...
var ImportExpression = class extends NodeBase {
  ...
  getDynamicImportMechanismAndHelper(resolution, exportMode, { compact, dynamicImportInCjs, format, generatedCode: { arrowFunctions }, interop }, { _, getDirectReturnFunction, getDirectReturnIifeLeft }, pluginDriver) {
    ...
    switch (format) {
      ...
      case "amd": {
        const resolve3 = compact ? "c" : "resolve";
        ...
        const resolveNamespace = helper ? `${resolveLeft}${resolve3}(/*#__PURE__*/${helper}(m))${resolveRight}` : resolve3;
        const [handlerLeft, handlerRight] = getDirectReturnFunction([resolve3, reject], {
          functionReturn: false,
          lineBreakIndent: null,
          name: null
        });
...
var MetaProperty = class extends NodeBase {
  ...
  render(code, renderOptions) {
    ...
    if (referenceId) {
      ...
      const relativePath = normalize((0, import_node_path2.relative)((0, import_node_path2.dirname)(chunkId), fileName));
...
var ModuleScope = class extends ChildScope {
  constructor(parent, context) {
    super(parent, context);
    this.variables.set("this", new LocalVariable(
      "this",
      null,
...
function getAttributesFromImportExpression(node) {
  ...
  const assertProperty = (_a = options.properties.find((property) => ATTRIBUTE_KEYWORDS.has(getPropertyKey(property)))) == null ? void 0 : _a.value;
  if (!assertProperty) {
    return EMPTY_OBJECT;
  }
...
var Module = class _Module {
  ...
  basename() {
    const base2 = (0, import_node_path2.basename)(this.id);
    const extension = (0, import_node_path2.extname)(this.id);
...
function amd(magicString, { accessedGlobals, dependencies, exports, hasDefaultExport, hasExports, id, indent: t, intro, isEntryFacade, isModuleFacade, namedExportsMode, log, outro, snippets }, { amd: amd2, esModule, externalLiveBindings, freeze, generatedCode: { symbols }, interop, strict }) {
...
function umd(magicString, { accessedGlobals, dependencies, exports, hasDefaultExport, hasExports, id, indent: t, intro, namedExportsMode, log, outro, snippets }, { amd: amd2, compact, esModule, extend, externalLiveBindings, freeze, interop, name, generatedCode: { symbols }, globals, noConflict, strict }) {
...
function makeUnique(name, { [lowercaseBundleKeys]: reservedLowercaseBundleKeys }) {
  if (!reservedLowercaseBundleKeys.has(name.toLowerCase()))
    return name;
  const extension = (0, import_node_path2.extname)(name);
...
var Chunk2 = class _Chunk {
  ...
  getPreliminaryFileName() {
    ...
    if (file) {
      fileName = (0, import_node_path2.basename)(file);
...
`);
  ...
  getPreserveModulesChunkNameFromModule(module) {
    ...
    const extensionName = (0, import_node_path2.extname)(sanitizedId);
    ...
    if (isAbsolute(idWithoutExtension)) {
      return preserveModulesRoot && (0, import_node_path2.resolve)(idWithoutExtension).startsWith(preserveModulesRoot) ? idWithoutExtension.slice(preserveModulesRoot.length).replace(/^[/\\]/, "") : relative(this.inputBase, idWithoutExtension);
    } else {
      return `_virtual/${(0, import_node_path2.basename)(idWithoutExtension)}`;
    }
  }
...
function commondir(files) {
  if (files.length === 0)
    return "/";
  if (files.length === 1)
    return (0, import_node_path2.dirname)(files[0]);
...
function getCollapsedSourcemap(id, originalCode, originalSourcemap, sourcemapChain, linkMap) {
  ...
  if (originalSourcemap) {
    ...
    const directory = (0, import_node_path2.dirname)(id) || ".";
    const sourceRoot = originalSourcemap.sourceRoot || ".";
    const baseSources = sources.map((source2, index) => new Source((0, import_node_path2.resolve)(directory, sourceRoot, source2), sourcesContent[index]));
...
function collapseSourcemaps(file, map, modules, bundleSourcemapChain, excludeContent, log) {
  ...
  if (file) {
    const directory = (0, import_node_path2.dirname)(file);
    sources = sources.map((source2) => (0, import_node_path2.relative)(directory, source2));
    file = (0, import_node_path2.basename)(file);
  }
...
function getXxhash(input) {
  ...
  return (0, import_native2.xxhashBase64Url)(buffer);
}
...
async function transformChunk(magicString, fileName, usedModules, chunkGraph, options, outputPluginDriver, log) {
  ...
  if (sourcemap) {
    ...
    let resultingFile;
    if (file)
      resultingFile = (0, import_node_path2.resolve)(sourcemapFile || file);
    else if (dir)
      resultingFile = (0, import_node_path2.resolve)(dir, fileName);
    else
      resultingFile = (0, import_node_path2.resolve)(fileName);
...
function emitSourceMapAndGetComment(fileName, map, pluginDriver, { sourcemap, sourcemapBaseUrl }) {
  ...
  if (sourcemap === "inline") {
    ...
    const sourcemapFileName = (0, import_node_path2.basename)(fileName);
...
async function findFile(file, preserveSymlinks) {
  try {
    ...
    if (preserveSymlinks && stats.isSymbolicLink() || stats.isFile()) {
      ...
      const files = await (0, import_promises.readdir)((0, import_node_path2.dirname)(file));
...
var ModuleLoader = class {
  ...
  getResolveDynamicImportPromises(module) {
    return module.dynamicImports.map(async (dynamicImport) => {
      const resolvedId = await this.resolveDynamicImport(module, typeof dynamicImport.argument === "string" ? dynamicImport.argument : dynamicImport.argument.esTreeNode, module.id, getAttributesFromImportExpression(dynamicImport.node));
...
function normalizeRelativeExternalId(source, importer) {
  return isRelative(source) ? importer ? (0, import_node_path2.resolve)(importer, "..", source) : (0, import_node_path2.resolve)(source) : source;
}
...
var Queue = class {
  ...
  run(task) {
    return new Promise((resolve3, reject) => {
      this.queue.push({ reject, resolve: resolve3, task });
      this.work();
    });
  }
  async work() {
    ...
    while (entry = this.queue.shift()) {
      const { reject, resolve: resolve3, task } = entry;
      try {
...
var getModuleContext = (config, context) => {
  ...
  if (configModuleContext) {
    ...
    for (const [key, moduleContext] of Object.entries(configModuleContext)) {
      contextByModuleId[(0, import_node_path2.resolve)(key)] = moduleContext;
    }
...
var getPreserveModulesRoot = (config) => {
  ...
  return (0, import_node_path2.resolve)(preserveModulesRoot);
};
var getAmd = (config) => {
  const mergedOption = {
    autoId: false,
...
async function writeOutputFile(outputFile, outputOptions) {
  ...
  await (0, import_promises.mkdir)((0, import_node_path2.dirname)(fileName), { recursive: true });
...
async function watchInternal(configs, emitter) {
  ...
  await loadFsEvents();
  const { Watcher } = await import("./watch-GXAKECQQ.js");
...
rollup/dist/es/shared/node-entry.js:
  ...
  *)
*/
//# sourceMappingURL=chunk-GNKY3IWT.js.map
